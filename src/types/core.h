...
tulip_value  name(char** modulePath, unsigned int modulePathLen, char* string);

tulip_value  tag(char* name, unsigned int length, tulip_value arguments[]);
bool         validate_tag_ast(tulip_value subject);

tulip_value  block(tulip_value contents[], unsigned int length);
...
tulip_value  branch(tulip_value* predicates, tulip_value* conseuqnces, unsigned int length);
